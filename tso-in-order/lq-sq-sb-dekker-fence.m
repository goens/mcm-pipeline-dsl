...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ write_result ===> await_committed || await_creation || await_creation" 
...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ build_packet_send_mem_request ===> build_packet_send_mem_request || build_packet_send_mem_request || await_mem_response || await_mem_response || await_creation || await_creation || build_packet_send_mem_request || await_mem_response || await_mem_response || await_creation || await_creation || await_creation || await_creation" 
...
begin
  ...
  if (next_state.core_[ j ].LQ_.entries[ i ].instruction.op = ld) then
    if is_instruction_on_any_state then
      ...
      while ((RENAME_offset < RENAME_difference) & ((RENAME_while_break = false) & (RENAME_found_entry = false))) do
        RENAME_curr_idx := ((RENAME_entry_idx + (RENAME_NUM_ENTRIES_CONST - RENAME_offset)) % RENAME_NUM_ENTRIES_CONST);
        if ((next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.seq_num < next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.op = ld)) then
          if (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].state = issue_if_head) then
      ...
      for IQ_iter : IQ_idx_t do
        if next_state.core_[ j ].IQ_.entries[ IQ_iter ].valid then
          if ((next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.seq_num < next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.op = ld)) then
      ...
      while ((LQ_offset < LQ_difference) & ((LQ_while_break = false) & (LQ_found_entry = false))) do
        ...
        if ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].instruction.seq_num < next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].instruction.op = ld)) then
          if ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_scheduled) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_translation) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_fwd_check) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_sb_fwd_check_response) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = build_packet_send_mem_request) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_mem_response) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_creation) | (next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = build_packet_send_mem_request)))))))) then
      ...
      while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
        ROB_curr_idx := ((ROB_entry_idx + (ROB_NUM_ENTRIES_CONST - ROB_offset)) % ROB_NUM_ENTRIES_CONST);
        if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = ld)) then
      ...
      is_instruction_on_any_state := (RENAME_Load_is_in_state_set | (IQ_Load_is_in_state_set | (LQ_Load_is_in_state_set | ROB_Load_is_in_state_set)));
      if (next_state.core_[ j ].LQ_.entries[ i ].instruction.op = ld) then
        if is_instruction_on_any_state then
          next_state.core_[ j ].LQ_.entries[ i ].state := build_packet_send_mem_request;
        else
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_if_head ===> rob_commit_if_head || rob_await_creation || rob_commit_if_head || rob_await_creation || rob_await_creation || rob_commit_if_head || rob_await_creation || rob_commit_if_head || rob_await_creation || rob_await_creation || rob_commit_if_head || rob_await_creation || rob_await_creation" 
