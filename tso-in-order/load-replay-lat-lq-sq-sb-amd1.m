...
ROB_state : enum {rob_await_executed, rob_commit_if_head, rob_await_creation, init_rob_entry, ROB_await_replay_complete, original_commit_rob_commit_if_head};
...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ build_packet_send_mem_request ===> build_packet_send_mem_request || await_mem_response || await_mem_response || await_creation || await_creation" 
(((Sta.core_[ j ].LQ_.entries[ i ].state = build_packet_send_mem_request) & !(Sta.core_[ j ].mem_interface_.out_busy)) & !(Sta.core_[ j ].mem_interface_.out_busy))
==>
...
begin
  ...
  while ((RENAME_offset < RENAME_difference) & ((RENAME_while_break = false) & (RENAME_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.seq_num < next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.op = mfence)) then
  ...
  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ROB_curr_idx := ((ROB_entry_idx + (ROB_NUM_ENTRIES_CONST - ROB_offset)) % ROB_NUM_ENTRIES_CONST);
    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = mfence)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_if_head) | (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_if_head)) then
...
ruleset j : cores_t; i : SB_idx_t do 
  rule "SB sb_await_send_mem_req ===> sb_await_send_mem_req || sb_await_send_mem_req || sb_await_mem_response || sb_await_mem_response || sb_await_send_mem_req || sb_await_mem_response || sb_await_mem_response" 
(((((Sta.core_[ j ].SB_.entries[ i ].state = sb_await_send_mem_req) & !(Sta.core_[ j ].mem_interface_.out_busy)) & !(Sta.core_[ j ].mem_interface_.out_busy)) & !(Sta.core_[ j ].mem_interface_.out_busy)) & !(Sta.core_[ j ].mem_interface_.out_busy))
==>
...
begin
  ...
  while ((RENAME_offset < RENAME_difference) & ((RENAME_while_break = false) & (RENAME_found_entry = false))) do
    RENAME_curr_idx := ((RENAME_entry_idx + (RENAME_NUM_ENTRIES_CONST - RENAME_offset)) % RENAME_NUM_ENTRIES_CONST);
    if ((next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.seq_num < next_state.core_[ j ].SB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.op = mfence)) then
      if (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].state = issue_if_head) then
        RENAME_mfence_is_in_state_set := true;
      else
        RENAME_mfence_is_in_state_set := false;
  ...
  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ROB_curr_idx := ((ROB_entry_idx + (ROB_NUM_ENTRIES_CONST - ROB_offset)) % ROB_NUM_ENTRIES_CONST);
    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].SB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = mfence)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_if_head) | (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_if_head)) then
        ROB_mfence_is_in_state_set := true;
      else
        ROB_mfence_is_in_state_set := false;
  ...
  if (next_state.core_[ j ].SB_.entries[ i ].instruction.op = st) then
    if is_instruction_on_any_state then
      ...
      if (found_entry = false) then
        ...
        if ((next_state.core_[ j ].SB_.entries[ found_idx ].state = sb_await_creation) | ((next_state.core_[ j ].SB_.entries[ found_idx ].state = sb_await_send_mem_req) | ((next_state.core_[ j ].SB_.entries[ found_idx ].state = sb_await_mem_response) | (next_state.core_[ j ].SB_.entries[ found_idx ].state = sb_await_send_mem_req)))) then
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_if_head ===> rob_commit_if_head || ROB_await_replay_complete || original_commit_rob_commit_if_head || rob_commit_if_head || ROB_await_replay_complete || original_commit_rob_commit_if_head || rob_commit_if_head || rob_await_creation || rob_await_creation" 
...
begin
  ...
  while ((LQ_offset < LQ_difference) & ((LQ_while_break = false) & (LQ_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].instruction.op = ld)) then
      if ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_scheduled) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_translation) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_fwd_check) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_sb_fwd_check_response) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = build_packet_send_mem_request) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_mem_response) | (next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = build_packet_send_mem_request))))))) then
  ...
  while ((SQ_offset < SQ_difference) & ((SQ_while_break = false) & (SQ_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].SQ_.entries[ SQ_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].SQ_.entries[ SQ_curr_idx ].instruction.op = st)) then
      if ((next_state.core_[ j ].SQ_.entries[ SQ_curr_idx ].state = sq_await_scheduled) | ((next_state.core_[ j ].SQ_.entries[ SQ_curr_idx ].state = sq_await_translation) | (next_state.core_[ j ].SQ_.entries[ SQ_curr_idx ].state = sq_await_committed))) then
        SQ_Store_is_in_state_set := true;
      else
        SQ_Store_is_in_state_set := false;
      end;
  ...
  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = st)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_executed) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_if_head) | (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_if_head))) then
        ROB_Store_is_in_state_set := true;
      else
        ROB_Store_is_in_state_set := false;
      end;
  ...
  is_instruction_on_any_state := (RENAME_Load_is_in_state_set | (IQ_Load_is_in_state_set | (LQ_Load_is_in_state_set | (RENAME_Store_is_in_state_set | (IQ_Store_is_in_state_set | (SQ_Store_is_in_state_set | (SB_Store_is_in_state_set | ROB_Store_is_in_state_set)))))));
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB original_commit_rob_commit_if_head ===> rob_await_creation || rob_commit_if_head || rob_await_creation || rob_await_creation || rob_await_creation || rob_commit_if_head || rob_await_creation || rob_await_creation || rob_await_creation || rob_await_creation" 
