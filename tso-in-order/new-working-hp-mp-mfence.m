...
function search_skid_buffer_seq_num_idx (
  skid_buffer_ : skid_buffer;
  seq_num : inst_count_t
) : skid_buffer_idx_t;
begin
  for i : skid_buffer_idx_t do
    if (skid_buffer_.entries[ i ].instruction.seq_num = seq_num) then
      return i;
    end;
  endfor;
  error "£ctrler_name Search: didn't find it? how? bad seq_num idx?";
...
begin
  ...
  if ((next_state.core_[ j ].SB_.entries[ i ].instruction.op = st) & is_instruction_on_any_state) then
    ...
    is_instruction_on_any_state := ROB_mfence_is_in_state_set;
    if ((next_state.core_[ j ].SB_.entries[ i ].instruction.op = st) & is_instruction_on_any_state) then
      next_state.core_[ j ].SB_.entries[ i ].state := sb_await_send_mem_req;
      return ;
    else
...
begin
  ...
  is_instruction_on_any_state := ROB_mfence_is_in_state_set;
  if ((next_state.core_[ j ].ROB_.entries[ i ].instruction.op = ldar) & is_instruction_on_any_state) then
    ...
    is_instruction_on_any_state := ROB_mfence_is_in_state_set;
    if ((next_state.core_[ j ].ROB_.entries[ i ].instruction.op = ld) & is_instruction_on_any_state) then
      next_state.core_[ j ].ROB_.entries[ i ].state := rob_commit_if_head;
      return ;
    else
      ...
      if ((next_state.core_[ j ].ROB_.entries[ i ].instruction.op = mfence) & is_instruction_on_any_state) then
        next_state.core_[ j ].ROB_.entries[ i ].state := rob_commit_if_head;
        return ;
      else
