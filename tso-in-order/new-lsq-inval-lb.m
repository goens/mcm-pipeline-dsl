...
begin
  ...
  alias rename_c1 : init_state.core_[ 1 ].RENAME_ do
    ...
    rename_c1.entries[ 2 ].instruction.dest_reg := 0;
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ lsq_build_packet_send_mem_request ===> lsq_await_load_mem_response || lsq_await_creation" 
...
begin
  ...
  if ((next_state.core_[ j ].ROB_.entries[ i ].instruction.op = st) & is_instruction_on_any_state) then
