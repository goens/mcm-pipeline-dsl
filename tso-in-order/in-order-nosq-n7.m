  ...
  if (mem_interface.in_msg.r_w = read) then
    ...
    if (mem_interface.in_msg.store_state = await_handling) then
      ...
      if 
        ...
        error "Got completed store, expected second memory stage to await it!";
      endif;
    end;
  end;
...
==>
  ...
  var IQ_mfence_is_in_state_set : boolean;
  var memory_unit_sender_mfence_is_in_state_set : boolean;
  var is_instruction_on_any_state : boolean;
  var ROB_while_break : boolean;
  var ROB_found_entry : boolean;
  var ROB_entry_idx : ROB_idx_t;
...
begin
  ...
  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ROB_curr_idx := ((ROB_entry_idx + (ROB_NUM_ENTRIES_CONST - ROB_offset)) % ROB_NUM_ENTRIES_CONST);
    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].second_memory_stage_.instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = mfence)) then
  ...
  IQ_mfence_is_in_state_set := false;
  found_entry := false;
  for IQ_iter : IQ_idx_t do
    if next_state.core_[ j ].IQ_.entries[ IQ_iter ].valid then
      if ((next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.seq_num < next_state.core_[ j ].second_memory_stage_.instruction.seq_num) & (next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.op = mfence)) then
        if (found_entry = false) then
  ...
  if (found_entry = false) then
    ...
    if ((next_state.core_[ j ].IQ_.entries[ found_idx ].state = iq_schedule_inst) | (next_state.core_[ j ].IQ_.entries[ found_idx ].state = iq_schedule_inst)) then
      IQ_mfence_is_in_state_set := true;
    else
      IQ_mfence_is_in_state_set := false;
    end;
  end;
  memory_unit_sender_mfence_is_in_state_set := false;
  if ((next_state.core_[ j ].memory_unit_sender_.instruction.op = mfence) & (next_state.core_[ j ].memory_unit_sender_.instruction.seq_num < next_state.core_[ j ].second_memory_stage_.instruction.seq_num)) then
    if ((next_state.core_[ j ].memory_unit_sender_.state = memory_unit_stage_send) | ((next_state.core_[ j ].memory_unit_sender_.state = memory_unit_receiver) | (next_state.core_[ j ].memory_unit_sender_.state = memory_unit_load_result_write))) then
      memory_unit_sender_mfence_is_in_state_set := true;
    else
      memory_unit_sender_mfence_is_in_state_set := false;
    end;
  else
  ...
  is_instruction_on_any_state := (ROB_mfence_is_in_state_set | (IQ_mfence_is_in_state_set | memory_unit_sender_mfence_is_in_state_set));
...
begin
  ...
  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ROB_curr_idx := ((ROB_entry_idx + (ROB_NUM_ENTRIES_CONST - ROB_offset)) % ROB_NUM_ENTRIES_CONST);
    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].IQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = mfence)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_if_head) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_based_on_inst) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_is_executed) | (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_based_on_inst)))) then
        ROB_mfence_is_in_state_set := true;
      else
        ROB_mfence_is_in_state_set := false;
  ...
  for IQ_iter : IQ_idx_t do
    if next_state.core_[ j ].IQ_.entries[ IQ_iter ].valid then
      if ((next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.seq_num < next_state.core_[ j ].IQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.op = mfence)) then
        if (found_entry = false) then
  ...
  memory_unit_sender_mfence_is_in_state_set := false;
  if ((next_state.core_[ j ].memory_unit_sender_.instruction.op = mfence) & (next_state.core_[ j ].memory_unit_sender_.instruction.seq_num < next_state.core_[ j ].IQ_.entries[ i ].instruction.seq_num)) then
    if ((next_state.core_[ j ].memory_unit_sender_.state = memory_unit_stage_send) | ((next_state.core_[ j ].memory_unit_sender_.state = memory_unit_receiver) | (next_state.core_[ j ].memory_unit_sender_.state = memory_unit_load_result_write))) then
      memory_unit_sender_mfence_is_in_state_set := true;
    else
      memory_unit_sender_mfence_is_in_state_set := false;
  ...
  if (next_state.core_[ j ].IQ_.entries[ i ].instruction.op = ld) then
    if is_instruction_on_any_state then
      ...
      while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
        ROB_curr_idx := ((ROB_entry_idx + (ROB_NUM_ENTRIES_CONST - ROB_offset)) % ROB_NUM_ENTRIES_CONST);
        if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].IQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = ld)) then
          if (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_is_executed) then
            ROB_Load_is_in_state_set := true;
          else
            ROB_Load_is_in_state_set := false;
      ...
      for IQ_iter : IQ_idx_t do
        if next_state.core_[ j ].IQ_.entries[ IQ_iter ].valid then
          if ((next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.seq_num < next_state.core_[ j ].IQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.op = ld)) then
            if (found_entry = false) then
              found_element := (next_state.core_[ j ].IQ_.entries[ i ].instruction.seq_num - next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.seq_num);
              found_idx := IQ_iter;
            else
      ...
      memory_unit_sender_Load_is_in_state_set := false;
      if ((next_state.core_[ j ].memory_unit_sender_.instruction.op = ld) & (next_state.core_[ j ].memory_unit_sender_.instruction.seq_num < next_state.core_[ j ].IQ_.entries[ i ].instruction.seq_num)) then
        if ((next_state.core_[ j ].memory_unit_sender_.state = memory_unit_stage_send) | (next_state.core_[ j ].memory_unit_sender_.state = memory_unit_receiver)) then
          memory_unit_sender_Load_is_in_state_set := true;
        else
          memory_unit_sender_Load_is_in_state_set := false;
