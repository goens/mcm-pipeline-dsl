...
(Sta.core_[ 2 ].LQ_.num_entries = 0)&
(Sta.core_[ 2 ].SB_.num_entries = 0) &
(Sta.core_[ 2 ].SQ_.num_entries = 0)
==>
...
begin
  ...
  while ((LQ_offset < LQ_difference) & ((LQ_while_break = false) & (LQ_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].instruction.op = ld)) then
      if ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_translation) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_fwd_check) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_sb_fwd_check_response) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = build_packet_send_mem_request) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_mem_response) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_scheduled) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = build_packet_send_mem_request) | (next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = build_packet_send_mem_request)))))))) then
