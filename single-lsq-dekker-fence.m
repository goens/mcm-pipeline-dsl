...
ROB_state : enum {rob_await_executed, rob_clear_lsq_store_head, rob_commit_time_await_st_mem_resp, rob_commit_po, rob_commit_if_head, rob_await_creation, init_rob_entry, ROB_stall_rob_commit_po};
          ...
          for ROB_squash_idx : ROB_idx_t do
            if true then
              if (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state = rob_await_executed) then
               ...
               elsif (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state = ROB_stall_rob_commit_po) then
                if ((next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].instruction.op = ld) & (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].instruction.seq_num > violating_seq_num)) then
                  next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].is_executed := false;
                  next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state := rob_await_executed;
                end;
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ LSQ_stall_lsq_build_packet_send_mem_request ===> LSQ_stall_lsq_build_packet_send_mem_request || lsq_build_packet_send_mem_request || lsq_build_packet_send_mem_request || lsq_ld_st_fwd_branch || lsq_ld_st_fwd_branch" 
...
begin
  ...
  while ((RENAME_offset <= RENAME_difference) & ((RENAME_while_break = false) & (RENAME_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.seq_num < next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.op = mfence)) then
  ...
  while ((ROB_offset <= ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ROB_curr_idx := ((ROB_entry_idx + (ROB_NUM_ENTRIES_CONST - ROB_offset)) % ROB_NUM_ENTRIES_CONST);
    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = mfence)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_creation) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_if_head) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = ROB_stall_rob_commit_po) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_po) | (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_executed))))) then
  ...
  for IQ_iter : IQ_idx_t do
    if (Sta.core_[ j ].IQ_.entries[ IQ_iter ].instruction.seq_num != 0) then
      if ((next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.seq_num < next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.op = mfence)) then
  ...
  while ((LSQ_offset <= LSQ_difference) & ((LSQ_while_break = false) & (LSQ_found_entry = false))) do
    LSQ_curr_idx := ((LSQ_entry_idx + (LSQ_NUM_ENTRIES_CONST - LSQ_offset)) % LSQ_NUM_ENTRIES_CONST);
    if ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.seq_num < next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.op = mfence)) then
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_po ===> rob_await_creation || rob_await_creation || rob_commit_time_await_st_mem_resp || rob_await_executed" 
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_if_head ===> ROB_stall_rob_commit_po || rob_commit_if_head || rob_await_executed" 
...
begin
  ...
  if (next_state.core_[ j ].ROB_.head = i) then
    next_state.core_[ j ].ROB_.entries[ i ].state := ROB_stall_rob_commit_po;
  else
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB ROB_stall_rob_commit_po ===> ROB_stall_rob_commit_po || rob_commit_po || rob_commit_po || rob_await_executed" 
(Sta.core_[ j ].ROB_.entries[ i ].state = ROB_stall_rob_commit_po)
==>
...
begin
  ...
  while ((ROB_offset <= ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = st)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_executed) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_if_head) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_po) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_time_await_st_mem_resp) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_creation) | (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = ROB_stall_rob_commit_po)))))) then
        ROB_is_in_state_set := true;
      else
        ROB_is_in_state_set := false;
      end;
