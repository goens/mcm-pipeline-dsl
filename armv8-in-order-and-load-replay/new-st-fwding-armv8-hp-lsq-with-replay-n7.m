...
skid_buffer_NUM_ENTRIES_ENUM_CONST : 2;
skid_buffer_NUM_ENTRIES_CONST : 3;
...
begin
  ...
  alias rename_c1 : init_state.core_[ 1 ].RENAME_ do
    ...
    rename_c1.head := 0;
    rename_c1.tail := 1 % (CORE_INST_NUM + 1);
    rename_c1.num_entries := 1;
  end;
  alias rename_c2 : init_state.core_[ 2 ].RENAME_ do
    rename_c2.entries[ 0 ].instruction.op := ld;
    rename_c2.entries[ 0 ].instruction.seq_num := 1;
    rename_c2.entries[ 0 ].instruction.dest_reg := 0;
    rename_c2.entries[ 0 ].instruction.imm := 1;
    rename_c2.entries[ 0 ].instruction.write_value := 0;
    rename_c2.entries[ 1 ].instruction.op := ld;
    rename_c2.entries[ 1 ].instruction.seq_num := 2;
    rename_c2.entries[ 1 ].instruction.dest_reg := 1;
    rename_c2.entries[ 1 ].instruction.imm := 0;
    rename_c2.entries[ 1 ].instruction.write_value := 0;
    rename_c2.head := 0;
    rename_c2.tail := 2 % (CORE_INST_NUM + 1);
    rename_c2.num_entries := 2;
  end;
  return init_state;
end;
...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ write_result ===> LoadReplay_await_replay_start || await_creation" 
...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ replay_issue_load_to_mem ===> await_committed || replay_generated_await_mem_response || await_creation" 
((Sta.core_[ j ].LQ_.entries[ i ].state = replay_issue_load_to_mem) & !(Sta.core_[ j ].mem_interface_.out_busy))
==>
 
  var replay_load_addr_var : addr_idx_t;
  var no_fwding_st_found_bool__ : boolean;
...
begin
  ...
  if (found_entry = false) then
    ...
    replay_load_addr_var := next_state.core_[ j ].load_address_table_.entries[ found_idx ].lat_address;
    no_fwding_st_found_bool__ := false;
    found_entry := false;
    for SB_iter : SB_idx_t do
      if next_state.core_[ j ].SB_.entries[ SB_iter ].valid then
        if (((next_state.core_[ j ].SB_.entries[ SB_iter ].instruction.seq_num < next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num) & (replay_load_addr_var = next_state.core_[ j ].SB_.entries[ SB_iter ].phys_addr)) & ((next_state.core_[ j ].SB_.entries[ SB_iter ].instruction.op = st) | (next_state.core_[ j ].SB_.entries[ SB_iter ].instruction.op = stlr))) then
          if (found_entry = false) then
            found_element := (next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num - next_state.core_[ j ].SB_.entries[ SB_iter ].instruction.seq_num);
            found_idx := SB_iter;
          else
    ...
    if (found_entry = false) then
      no_fwding_st_found_bool__ := true;
    elsif (found_entry = true) then
      old_load_value := next_state.core_[ j ].rf_.rf[ next_state.core_[ j ].LQ_.entries[ i ].instruction.dest_reg ];
      if (old_load_value != next_state.core_[ j ].SB_.entries[ found_idx ].write_value) then
        next_state.core_[ j ].rf_.rf[ next_state.core_[ j ].LQ_.entries[ i ].instruction.dest_reg ] := next_state.core_[ j ].SB_.entries[ found_idx ].write_value;
    ...
    if no_fwding_st_found_bool__ then
...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ replay_compare_and_check_state ===> await_committed || await_creation" 
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_if_head ===> rob_commit_if_head || ROB_await_replay_complete || original_commit_rob_commit_if_head || rob_await_creation" 
...
begin
  ...
  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ROB_curr_idx := ((ROB_entry_idx + (ROB_NUM_ENTRIES_CONST - ROB_offset)) % ROB_NUM_ENTRIES_CONST);
    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = dmb_ld)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_if_head) | (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_executed)) then
  ...
  is_instruction_on_any_state := (ROB_dmb_sy_is_in_state_set | ROB_dmb_ld_is_in_state_set);
    ...
    if is_instruction_on_any_state then
      ...
      if (next_state.core_[ j ].ROB_.entries[ i ].instruction.op = ld) then
        if is_instruction_on_any_state then
          ...
          for IQ_iter : IQ_idx_t do
            if next_state.core_[ j ].IQ_.entries[ IQ_iter ].valid then
              if ((next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.op = ldar)) then
                if (found_entry = false) then
                  found_element := (next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num - next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.seq_num);
                  found_idx := IQ_iter;
                else
          ...
          while ((LQ_offset < LQ_difference) & ((LQ_while_break = false) & (LQ_found_entry = false))) do
            LQ_curr_idx := ((LQ_entry_idx + (LQ_NUM_ENTRIES_CONST - LQ_offset)) % LQ_NUM_ENTRIES_CONST);
            if ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].instruction.op = ldar)) then
              if ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_translation) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_fwd_check) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_sb_fwd_check_response) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = build_packet_send_mem_request) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_mem_response) | (next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_scheduled)))))) then
                LQ_LDAR_is_in_state_set := true;
              else
                LQ_LDAR_is_in_state_set := false;
          ...
          while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
            ROB_curr_idx := ((ROB_entry_idx + (ROB_NUM_ENTRIES_CONST - ROB_offset)) % ROB_NUM_ENTRIES_CONST);
            if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = ldar)) then
              if (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_executed) then
                ROB_LDAR_is_in_state_set := true;
              else
                ROB_LDAR_is_in_state_set := false;
          ...
          if (found_entry = false) then
            ...
            if ((next_state.core_[ j ].SB_.entries[ found_idx ].state = sb_await_send_mem_req) | ((next_state.core_[ j ].SB_.entries[ found_idx ].state = sb_await_mem_response) | ((next_state.core_[ j ].SB_.entries[ found_idx ].state = sb_await_send_mem_req) | (next_state.core_[ j ].SB_.entries[ found_idx ].state = sb_await_send_mem_req)))) then
              SB_Store_is_in_state_set := true;
            else
              SB_Store_is_in_state_set := false;
            end;
          ...
          while ((SQ_offset < SQ_difference) & ((SQ_while_break = false) & (SQ_found_entry = false))) do
            SQ_curr_idx := ((SQ_entry_idx + (SQ_NUM_ENTRIES_CONST - SQ_offset)) % SQ_NUM_ENTRIES_CONST);
            if ((next_state.core_[ j ].SQ_.entries[ SQ_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].SQ_.entries[ SQ_curr_idx ].instruction.op = stlr)) then
              if ((next_state.core_[ j ].SQ_.entries[ SQ_curr_idx ].state = sq_await_translation) | ((next_state.core_[ j ].SQ_.entries[ SQ_curr_idx ].state = sq_await_committed) | (next_state.core_[ j ].SQ_.entries[ SQ_curr_idx ].state = sq_await_scheduled))) then
                SQ_STLR_is_in_state_set := true;
              else
                SQ_STLR_is_in_state_set := false;
          ...
          for SB_iter : SB_idx_t do
            if next_state.core_[ j ].SB_.entries[ SB_iter ].valid then
              if ((next_state.core_[ j ].SB_.entries[ SB_iter ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].SB_.entries[ SB_iter ].instruction.op = stlr)) then
                if (found_entry = false) then
                  found_element := (next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num - next_state.core_[ j ].SB_.entries[ SB_iter ].instruction.seq_num);
                  found_idx := SB_iter;
                else
          ...
          is_instruction_on_any_state := (IQ_Load_is_in_state_set | (LQ_Load_is_in_state_set | (ROB_Load_is_in_state_set | (IQ_LDAR_is_in_state_set | (LQ_LDAR_is_in_state_set | (ROB_LDAR_is_in_state_set | (IQ_Store_is_in_state_set | (SQ_Store_is_in_state_set | (SB_Store_is_in_state_set | (ROB_Store_is_in_state_set | (IQ_STLR_is_in_state_set | (SQ_STLR_is_in_state_set | (SB_STLR_is_in_state_set | ROB_STLR_is_in_state_set)))))))))))));
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB original_commit_rob_commit_if_head ===> rob_await_creation || rob_commit_if_head" 
...
(Sta.core_[ 1 ].SB_.num_entries = 0) &
(Sta.core_[ 1 ].ROB_.num_entries = 0))) &
(Sta.core_[ 2 ].RENAME_.num_entries = 0) &
(Sta.core_[ 2 ].skid_buffer_.num_entries = 0) &
(Sta.core_[ 2 ].SB_.num_entries = 0) &
(Sta.core_[ 2 ].ROB_.num_entries = 0)
->
!((((Sta.core_[ 0 ].rf_.rf[ 0 ] = 1) &
(Sta.core_[ 0 ].rf_.rf[ 1 ] = 0)) &
((Sta.core_[ 2 ].rf_.rf[ 0 ] = 1) &
(Sta.core_[ 2 ].rf_.rf[ 1 ] = 0)))));
