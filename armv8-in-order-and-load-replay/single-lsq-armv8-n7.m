...
LSQ_state : enum {lsq_st_await_committed, lsq_ld_await_committed, lsq_ld_write_result, lsq_await_load_mem_response, lsq_build_packet_send_mem_request, lsq_ld_st_fwd_branch, lsq_ld_st_await_translation, lsq_await_scheduled, lsq_await_creation, init_default_state, LoadReplay_await_replay_start, replay_issue_load_to_mem, replay_generated_lsq_await_load_mem_response, replay_compare_and_check_state};
...
ROB_state : enum {rob_await_executed, rob_clear_lsq_store_head, rob_commit_time_await_st_mem_resp, rob_commit_po, rob_commit_if_head, rob_await_creation, init_rob_entry, ROB_await_replay_complete, original_commit_rob_commit_po};
...
(Sta.core_[ 2 ].IQ_.num_entries = 0) &
(Sta.core_[ 2 ].LSQ_.num_entries = 0)
==>
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ lsq_ld_write_result ===> LoadReplay_await_replay_start || lsq_await_creation" 
...
begin
  ...
  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = dmb_sy)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_po) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_executed) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_po) | (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_po)))) then
    ...
    if is_instruction_on_any_state then
      ...
      while ((LSQ_offset < LSQ_difference) & ((LSQ_while_break = false) & (LSQ_found_entry = false))) do
        ...
        if ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.seq_num < next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.op = ldar)) then
          if (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state = lsq_await_load_mem_response) then
      ...
      is_instruction_on_any_state := (IQ_LDAR_is_in_state_set | (LSQ_LDAR_is_in_state_set | ROB_LDAR_is_in_state_set));
      if (next_state.core_[ j ].LSQ_.entries[ i ].instruction.op = ld) then
        if is_instruction_on_any_state then
          next_state.core_[ j ].LSQ_.entries[ i ].state := lsq_build_packet_send_mem_request;
        else
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ replay_issue_load_to_mem ===> replay_generated_lsq_await_load_mem_response || lsq_await_creation" 
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ replay_compare_and_check_state ===> lsq_ld_await_committed || lsq_await_creation" 
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_po ===> rob_commit_po || ROB_await_replay_complete || original_commit_rob_commit_po || rob_await_creation" 
...
begin
  ...
  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = dmb_sy)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_po) | (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_executed)) then
        ROB_dmb_sy_is_in_state_set := true;
      else
        ROB_dmb_sy_is_in_state_set := false;
      end;
                ...
                if is_instruction_on_any_state then
                  ...
                  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
                    ...
                    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = st)) then
                      if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_time_await_st_mem_resp) | (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_executed)) then
                  ...
                  is_instruction_on_any_state := (IQ_Store_is_in_state_set | (ROB_Store_is_in_state_set | (IQ_STLR_is_in_state_set | ROB_STLR_is_in_state_set)));
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB original_commit_rob_commit_po ===> rob_await_creation || rob_commit_time_await_st_mem_resp" 
...
begin
  ...
  if ((next_state.core_[ j ].skid_buffer_.head = i) & (!((next_state.core_[ j ].skid_buffer_.num_entries = 0)) & !((next_state.core_[ j ].ROB_.num_entries = ROB_NUM_ENTRIES_CONST)))) then
    if ((next_state.core_[ j ].skid_buffer_.entries[ i ].instruction.op = mfence) | ((next_state.core_[ j ].skid_buffer_.entries[ i ].instruction.op = dmb_sy) | ((next_state.core_[ j ].skid_buffer_.entries[ i ].instruction.op = dmb_ld) | (next_state.core_[ j ].skid_buffer_.entries[ i ].instruction.op = dmb_st)))) then
      ...
      if ((next_state.core_[ j ].skid_buffer_.entries[ i ].instruction.op = mfence) | ((next_state.core_[ j ].skid_buffer_.entries[ i ].instruction.op = dmb_sy) | ((next_state.core_[ j ].skid_buffer_.entries[ i ].instruction.op = dmb_st) | (next_state.core_[ j ].skid_buffer_.entries[ i ].instruction.op = dmb_ld)))) then
        next_state.core_[ j ].ROB_.entries[ next_state.core_[ j ].ROB_.tail ].state := rob_commit_if_head;
      else
        next_state.core_[ j ].ROB_.entries[ next_state.core_[ j ].ROB_.tail ].state := rob_await_executed;
      end;
      ...
      if (!((next_state.core_[ j ].IQ_.num_entries = IQ_NUM_ENTRIES_CONST)) & !((next_state.core_[ j ].LSQ_.num_entries = LSQ_NUM_ENTRIES_CONST))) then
        ...
        next_state.core_[ j ].ROB_.tail := ((next_state.core_[ j ].ROB_.tail + 1) % ROB_NUM_ENTRIES_CONST);
        next_state.core_[ j ].ROB_.num_entries := (next_state.core_[ j ].ROB_.num_entries + 1);
        next_state.core_[ j ].LSQ_.entries[ next_state.core_[ j ].LSQ_.tail ].instruction := next_state.core_[ j ].skid_buffer_.entries[ i ].instruction;
        next_state.core_[ j ].LSQ_.entries[ next_state.core_[ j ].LSQ_.tail ].state := lsq_await_scheduled;
        next_state.core_[ j ].LSQ_.tail := ((next_state.core_[ j ].LSQ_.tail + 1) % LSQ_NUM_ENTRIES_CONST);
        next_state.core_[ j ].LSQ_.num_entries := (next_state.core_[ j ].LSQ_.num_entries + 1);
        next_state.core_[ j ].skid_buffer_.entries[ next_state.core_[ j ].skid_buffer_.head ].instruction.op := inval;
