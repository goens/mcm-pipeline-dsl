    ...
    if is_instruction_on_any_state then
      ...
      while ((LQ_offset < LQ_difference) & ((LQ_while_break = false) & (LQ_found_entry = false))) do
        ...
        if ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].instruction.seq_num < next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].instruction.op = ldar)) then
          if ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_translation) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_fwd_check) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_sb_fwd_check_response) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = build_packet_send_mem_request) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_mem_response) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_creation) | (next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_scheduled))))))) then
...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ replay_issue_load_to_mem ===> replay_generated_await_mem_response || await_creation" 
...
begin
  ...
  if (next_state.core_[ j ].SB_.entries[ i ].instruction.op = st) then
    if is_instruction_on_any_state then
      ...
      while ((RENAME_offset < RENAME_difference) & ((RENAME_while_break = false) & (RENAME_found_entry = false))) do
        RENAME_curr_idx := ((RENAME_entry_idx + (RENAME_NUM_ENTRIES_CONST - RENAME_offset)) % RENAME_NUM_ENTRIES_CONST);
        if ((next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.seq_num < next_state.core_[ j ].SB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.op = dmb_st)) then
          if (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].state = issue_if_head) then
      ...
      if (next_state.core_[ j ].SB_.entries[ i ].instruction.op = st) then
        if is_instruction_on_any_state then
          ...
          while ((RENAME_offset < RENAME_difference) & ((RENAME_while_break = false) & (RENAME_found_entry = false))) do
            RENAME_curr_idx := ((RENAME_entry_idx + (RENAME_NUM_ENTRIES_CONST - RENAME_offset)) % RENAME_NUM_ENTRIES_CONST);
            if ((next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.seq_num < next_state.core_[ j ].SB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.op = stlr)) then
              if (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].state = issue_if_head) then
          ...
          is_instruction_on_any_state := (RENAME_Store_is_in_state_set | (IQ_Store_is_in_state_set | (SQ_Store_is_in_state_set | (SB_Store_is_in_state_set | (ROB_Store_is_in_state_set | (RENAME_STLR_is_in_state_set | (IQ_STLR_is_in_state_set | (SQ_STLR_is_in_state_set | (SB_STLR_is_in_state_set | ROB_STLR_is_in_state_set)))))))));
...
begin
  ...
  while ((RENAME_offset < RENAME_difference) & ((RENAME_while_break = false) & (RENAME_found_entry = false))) do
    RENAME_curr_idx := ((RENAME_entry_idx + (RENAME_NUM_ENTRIES_CONST - RENAME_offset)) % RENAME_NUM_ENTRIES_CONST);
    if ((next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.op = dmb_ld)) then
      if (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].state = issue_if_head) then
  ...
  is_instruction_on_any_state := (RENAME_dmb_sy_is_in_state_set | (ROB_dmb_sy_is_in_state_set | (RENAME_dmb_ld_is_in_state_set | ROB_dmb_ld_is_in_state_set)));
...
invariant "n7"
(((((Sta.core_[ 0 ].RENAME_.num_entries = 0) & (Sta.core_[ 0 ].ROB_.num_entries = 0)) & ((Sta.core_[ 1 ].RENAME_.num_entries = 0) & (Sta.core_[ 1 ].ROB_.num_entries = 0))) & ((Sta.core_[ 2 ].RENAME_.num_entries = 0) & (Sta.core_[ 2 ].ROB_.num_entries = 0))
& (Sta.core_[ 0 ].SB_.num_entries = 0) & (Sta.core_[ 1 ].SB_.num_entries = 0)) -> !((((Sta.core_[ 0 ].rf_.rf[ 0 ] = 1) & (Sta.core_[ 0 ].rf_.rf[ 1 ] = 0)) & ((Sta.core_[ 2 ].rf_.rf[ 0 ] = 1) & (Sta.core_[ 2 ].rf_.rf[ 1 ] = 0)))));
