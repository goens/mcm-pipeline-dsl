...
LSQ_state : enum {lsq_st_await_committed, lsq_squashed_await_ld_mem_resp, lsq_ld_await_committed, lsq_ld_write_result, lsq_await_load_mem_response, lsq_build_packet_send_mem_request, lsq_ld_st_fwd_branch, lsq_await_translation, lsq_await_scheduled, lsq_await_creation, init_default_state};
...
begin
  ...
  if (mem_interface.in_msg.r_w = read) then
    ...
    if (mem_interface.in_msg.store_state = await_handling) then
      ...
      next_state.core_[ j ].invalidation_listener_.seq_num := mem_interface.in_msg.seq_num;
      next_state.core_[ j ].invalidation_listener_.address := mem_interface.in_msg.addr;
...
begin
  next_state := Sta;
  next_state.core_[ j ].invalidation_listener_.seq_num := 0;
  next_state.core_[ j ].invalidation_listener_.address := 0;
...
==>
  ...
  var LSQ_search_all_curr_idx : LSQ_idx_t;
...
begin
  ...
  LSQ_offset := 0;
  while ((LSQ_offset <= LSQ_difference) & (LSQ_while_break = false)) do
    LSQ_search_all_curr_idx := ((LSQ_entry_idx + (LSQ_NUM_ENTRIES_CONST - LSQ_offset)) % LSQ_NUM_ENTRIES_CONST);
    if ((next_state.core_[ j ].LSQ_.entries[ LSQ_search_all_curr_idx ].instruction.seq_num < Sta.core_[j].invalidation_listener_.seq_num) & (next_state.core_[ j ].LSQ_.entries[ LSQ_search_all_curr_idx ].instruction.op = ld)) then
      if ((next_state.core_[ j ].LSQ_.entries[ LSQ_search_all_curr_idx ].state = lsq_build_packet_send_mem_request) | ((next_state.core_[ j ].LSQ_.entries[ LSQ_search_all_curr_idx ].state = lsq_await_load_mem_response) | ((next_state.core_[ j ].LSQ_.entries[ LSQ_search_all_curr_idx ].state = lsq_ld_write_result) | ((next_state.core_[ j ].LSQ_.entries[ LSQ_search_all_curr_idx ].state = lsq_ld_await_committed) | (next_state.core_[ j ].LSQ_.entries[ LSQ_search_all_curr_idx ].state = lsq_squashed_await_ld_mem_resp))))) then
  ...
  while ((ROB_offset <= ROB_difference) & (ROB_while_break = false)) do
    ...
    if ((next_state.core_[ j ].ROB_.entries[ ROB_search_all_curr_idx ].instruction.seq_num < Sta.core_[j].invalidation_listener_.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_search_all_curr_idx ].instruction.op = ld)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_search_all_curr_idx ].state = rob_await_executed) | ((next_state.core_[ j ].ROB_.entries[ ROB_search_all_curr_idx ].state = rob_commit_if_head) | (next_state.core_[ j ].ROB_.entries[ ROB_search_all_curr_idx ].state = rob_commit_po))) then
        for ROB_squash_idx : ROB_idx_t do
          if true then
          end;
        endfor;
