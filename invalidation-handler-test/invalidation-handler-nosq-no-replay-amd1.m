...
begin
  ...
  --   (Sta.core_[j].memory_unit_sender_.instruction.seq_num = mem_interface.in_msg.seq_num) then
  --   put "CORE-ACK: REPLAY 1st Mem unit is the recipient, check if load or store..\n";
  --   if (mem_interface.in_msg.r_w = read) then
  --     rob_id := search_ROB_seq_num_idx(rob, Sta.core_[j].memory_unit_sender_.instruction.seq_num);
  --     put "CORE-ACK: Rob id: (";
  --     put rob_id;
...
begin
  ...
  while ((ROB_offset <= ROB_difference) & (ROB_while_break = false)) do
    ...
    if ((next_state.core_[ j ].ROB_.entries[ ROB_search_all_curr_idx ].instruction.seq_num < Sta.core_[j].invalidation_listener_.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_search_all_curr_idx ].instruction.op = ld)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_search_all_curr_idx ].state = rob_await_is_executed) | ((next_state.core_[ j ].ROB_.entries[ ROB_search_all_curr_idx ].state = rob_commit_if_head) | (next_state.core_[ j ].ROB_.entries[ ROB_search_all_curr_idx ].state = rob_commit_based_on_inst))) then
        for ROB_squash_idx : ROB_idx_t do
          if true then
          end;
        endfor;
