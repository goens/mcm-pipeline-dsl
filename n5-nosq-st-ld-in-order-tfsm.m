...
ROB_state : enum {rob_complete_store, rob_wait_store_completed, rob_commit_based_on_inst, rob_commit_if_head, rob_await_is_executed, rob_await_creation, init_rob_entry};
...
IQ_state : enum {iq_schedule_inst, iq_await_creation, init_iq_entry, IQ_stall_iq_schedule_inst};
...
begin
  ...
  alias rename_c1 : init_state.core_[ 1 ].RENAME_ do
    ...
    rename_c1.entries[ 0 ].instruction.write_value := 2;
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_based_on_inst ===> rob_await_creation || rob_wait_store_completed" 
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_if_head ===> rob_commit_based_on_inst || rob_commit_if_head || rob_await_is_executed" 
...
ruleset j : cores_t; i : IQ_idx_t do 
  rule "IQ IQ_stall_iq_schedule_inst ===> IQ_stall_iq_schedule_inst || iq_schedule_inst || iq_schedule_inst || iq_schedule_inst" 
(Sta.core_[ j ].IQ_.entries[ i ].state = IQ_stall_iq_schedule_inst)
==>
  ...
  if (next_state.core_[ j ].IQ_.entries[ i ].instruction.op = ld) then
    ...
    for ROB_iter : ROB_idx_t do
      if (Sta.core_[ j ].ROB_.entries[ ROB_iter ].instruction.seq_num != 0) then
        if ((next_state.core_[ j ].ROB_.entries[ ROB_iter ].instruction.seq_num < next_state.core_[ j ].IQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_iter ].instruction.op = st)) then
          if (found_entry = false) then
            found_element := (next_state.core_[ j ].IQ_.entries[ i ].instruction.seq_num - next_state.core_[ j ].ROB_.entries[ ROB_iter ].instruction.seq_num);
            found_idx := ROB_iter;
          else
            if ((next_state.core_[ j ].IQ_.entries[ i ].instruction.seq_num - next_state.core_[ j ].ROB_.entries[ ROB_iter ].instruction.seq_num) < found_element) then
              found_element := (next_state.core_[ j ].IQ_.entries[ i ].instruction.seq_num - next_state.core_[ j ].ROB_.entries[ ROB_iter ].instruction.seq_num);
              found_idx := ROB_iter;
            end;
          end;
    ...
    if (found_entry = false) then
      ...
      if !((next_state.core_[ j ].ROB_.entries[ found_idx ].state = rob_complete_store)) then
        next_state.core_[ j ].IQ_.entries[ i ].state := IQ_stall_iq_schedule_inst;
      else
        next_state.core_[ j ].IQ_.entries[ i ].state := iq_schedule_inst;
      end;
    end;
    ...
    if (!((Sta.core_[ j ].IQ_.num_entries = IQ_NUM_ENTRIES_CONST)) & !((Sta.core_[ j ].ROB_.num_entries = ROB_NUM_ENTRIES_CONST))) then
      ...
      while ((IQ_offset <= IQ_difference) & ((IQ_loop_break = false) & ((IQ_found_entry = false) & (IQ_difference >= 0)))) do
        ...
        if (next_state.core_[ j ].IQ_.entries[ IQ_curr_idx ].state = iq_await_creation) then
          ...
          next_state.core_[ j ].IQ_.entries[ IQ_curr_idx ].state := IQ_stall_iq_schedule_inst;
          IQ_found_entry := true;
        end;
...
invariant "n5"
((((Sta.core_[ 0 ].RENAME_.num_entries = 0) & (Sta.core_[ 0 ].ROB_.num_entries = 0)) & ((Sta.core_[ 1 ].RENAME_.num_entries = 0) & (Sta.core_[ 1 ].ROB_.num_entries = 0))) -> !((((Sta.core_[ 0 ].rf_.rf[ 0 ] = 0) & (Sta.core_[ 0 ].rf_.rf[ 1 ] = 2)) & ((Sta.core_[ 1 ].rf_.rf[ 0 ] = 0) & (Sta.core_[ 1 ].rf_.rf[ 1 ] = 1)))));
