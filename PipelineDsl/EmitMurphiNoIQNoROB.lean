...
def compose_murphi_file_components_but_no_IQ_no_ROB
...
function iq_insert(
  ...
  while 0 < i do
    ...
    if ( iq_new .entries[i] .state = iq_await_creation )
      then
      iq_new .entries[i] .instruction := inst;
      --# TODO NOTE: insert as valid
      --# TODO NOTE: and have scoreboard
      --# mark as ready!
      iq_new .entries[i] .state := iq_schedule_inst;
      --# Finish, leave fn
...
def gen_murphi_litmus_test_programs_no_IQ_no_ROB
-- Consts, like num entries per buffer-type ctrler
( const_decls : List Murϕ.Decl)
-- Types, like ctrler defns
( type_decls : List Murϕ.Decl)
...
:=
  ...
  ActiveLitmusTests.map (
    λ litmus_test =>
      ...
      let program' := compose_murphi_file_components_but_no_IQ_no_ROB const_decls type_decls func_decls rules ctrler_list litmus_test
