...
partial def true_if_stmts_have_load_mem_access
(stmt : Statement)
:=
  -- dbg_trace "==BEGIN GET-TRANSITIONS ==\n"
  -- dbg_trace stmt
  ...
  | Statement.stray_expr expr' => 
    ...
    | Expr.some_term term' =>
      ...
      | Term.function_call qual_name lst_expr =>
        ...
        | QualifiedName.mk lst_idents' =>
          if ((lst_idents'.contains "memory_interface") &&
            (lst_idents'.contains "send_load_request"))
            then [true]
            else []
      | _ => []
    | _ => []
...
-- partial def true_if_stmts_global_load_labelled
-- (stmt : Statement)
-- : List Bool
-- :=
--   -- dbg_trace "==BEGIN GET-TRANSITIONS ==\n"
...
--     | Pipeline.Label => 2
--     if label == "global_perform" then
--       true_if_stmts_global_load_labelled stmt
--     else
--       []
--   | _ => []
