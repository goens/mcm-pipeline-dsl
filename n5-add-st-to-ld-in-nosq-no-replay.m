...
ruleset j : cores_t; i : IQ_idx_t do 
  rule "IQ IQ_stall_iq_schedule_inst ===> IQ_stall_iq_schedule_inst || iq_schedule_inst || iq_schedule_inst" 
...
begin
  ...
  for ROB_iter : ROB_idx_t do
    if (Sta.core_[ j ].ROB_.entries[ ROB_iter ].instruction.seq_num != 0) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_iter ].instruction.seq_num < next_state.core_[ j ].IQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_iter ].instruction.op = ld)) then
