...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ replay_compare_and_check_state ===> lsq_ld_await_committed || lsq_ld_st_fwd_branch || lsq_ld_st_fwd_branch" 
    ...
    for LSQ_squash_idx : LSQ_idx_t do
      if true then
        if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state = lsq_st_await_committed) then
          ...
          put next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state;
          error "Replay Controller is not on an expected state for a msg: (squash) from: (LSQ) to: (LSQ)";
        end;
      end;
    endfor;
  end;
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_po ===> ROB_await_replay_complete || original_commit_rob_commit_po || rob_await_executed" 
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB original_commit_rob_commit_po ===> rob_await_creation || rob_commit_time_await_st_mem_resp || rob_await_executed" 
