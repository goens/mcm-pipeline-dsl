...
ROB_state : enum {rob_clear_lsq_store_head, rob_commit_time_await_st_mem_resp, rob_commit_po, rob_commit_if_head, rob_await_executed, rob_await_creation, init_rob_entry, ROB_await_replay_complete, original_commit_rob_commit_po};
...
begin
  ...
  for i : 0 .. LSQ_NUM_ENTRIES_ENUM_CONST do
    ...
    if (curr_entry.instruction.seq_num = seq_num) then
      assert (
      (curr_entry.state = lsq_squashed_await_ld_mem_resp) | (curr_entry.state = lsq_await_load_mem_response)
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ lsq_ld_write_result ===> lsq_ld_await_committed || lsq_ld_st_fwd_branch" 
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ lsq_build_packet_send_mem_request ===> lsq_await_load_mem_response || lsq_ld_st_fwd_branch" 
...
==>
  ...
  var squash_ld_id : LSQ_idx_t;
  var squash_st_id : LSQ_idx_t;
  var loop_break : boolean;
  var difference : LSQ_idx_t;
  var entry_idx : LSQ_idx_t;
  var offset : LSQ_idx_t;
  var curr_idx : LSQ_idx_t;
...
begin
  ...
  if (next_state.core_[ j ].LSQ_.entries[ i ].instruction.op = ld) then
    ...
    while ((LSQ_offset <= LSQ_difference) & ((LSQ_while_break = false) & (LSQ_found_entry = false))) do
      ...
      if ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.op = st) & ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].phys_addr = next_state.core_[ j ].LSQ_.entries[ i ].phys_addr) & (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.seq_num < next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num))) then
    ...
    if (next_state.core_[ j ].LSQ_.entries[ i ].instruction.op = st) then
      ...
      entry_idx := next_state.core_[ j ].LSQ_.head;
      difference := ((((next_state.core_[ j ].LSQ_.tail + LSQ_NUM_ENTRIES_CONST) - 1) - entry_idx) % LSQ_NUM_ENTRIES_CONST);
      offset := 0;
      while ((offset <= difference) & ((loop_break = false) & (difference >= 0))) do
        curr_idx := ((entry_idx + offset) % LSQ_NUM_ENTRIES_CONST);
        if ((next_state.core_[ j ].LSQ_.entries[ curr_idx ].instruction.op = ld) & ((next_state.core_[ j ].LSQ_.entries[ curr_idx ].phys_addr = next_state.core_[ j ].LSQ_.entries[ i ].phys_addr) & (next_state.core_[ j ].LSQ_.entries[ curr_idx ].instruction.seq_num > next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num))) then
          violating_seq_num := next_state.core_[ j ].LSQ_.entries[ curr_idx ].instruction.seq_num;
          ...
          while ((squash_offset <= squash_diff) & (squash_diff >= 0)) do
            ...
            if (curr_rob_inst.op = ld) then
              squash_ld_id := search_LSQ_seq_num_idx(next_state.core_[ j ].LSQ_, curr_rob_inst.seq_num);
              if (next_state.core_[ j ].LSQ_.entries[ squash_ld_id ].state = lsq_ld_await_committed) then
                if (next_state.core_[ j ].LSQ_.entries[ squash_ld_id ].instruction.seq_num >= violating_seq_num) then
                  rob := next_state.core_[ j ].ROB_;
                  rob_id := search_ROB_seq_num_idx(rob, next_state.core_[ j ].LSQ_.entries[ squash_ld_id ].instruction.seq_num);
                  ...
                  next_state.core_[ j ].LSQ_.entries[ squash_ld_id ].state := lsq_ld_st_fwd_branch;
                end;
              elsif (next_state.core_[ j ].LSQ_.entries[ squash_ld_id ].state = lsq_ld_write_result) then
                if (next_state.core_[ j ].LSQ_.entries[ squash_ld_id ].instruction.seq_num >= violating_seq_num) then
                  next_state.core_[ j ].LSQ_.entries[ squash_ld_id ].state := lsq_ld_st_fwd_branch;
                end;
               elsif (next_state.core_[ j ].LSQ_.entries[ squash_ld_id ].state = lsq_await_load_mem_response) then
                if (next_state.core_[ j ].LSQ_.entries[ squash_ld_id ].instruction.seq_num >= violating_seq_num) then
                  next_state.core_[ j ].LSQ_.entries[ squash_ld_id ].state := lsq_squashed_await_ld_mem_resp;
                end;
               elsif (next_state.core_[ j ].LSQ_.entries[ squash_ld_id ].state = lsq_build_packet_send_mem_request) then
                if (next_state.core_[ j ].LSQ_.entries[ squash_ld_id ].instruction.seq_num >= violating_seq_num) then
                  next_state.core_[ j ].LSQ_.entries[ squash_ld_id ].state := lsq_ld_st_fwd_branch;
                end;
              end;
            elsif (curr_rob_inst.op = st) then
              squash_st_id := search_LSQ_seq_num_idx(next_state.core_[ j ].LSQ_, curr_rob_inst.seq_num);
              if (next_state.core_[ j ].LSQ_.entries[ squash_st_id ].state = lsq_ld_await_committed) then
                if (next_state.core_[ j ].LSQ_.entries[ squash_st_id ].instruction.seq_num >= violating_seq_num) then
                  rob := next_state.core_[ j ].ROB_;
                  rob_id := search_ROB_seq_num_idx(rob, next_state.core_[ j ].LSQ_.entries[ squash_st_id ].instruction.seq_num);
                  assert (rob.entries[ rob_id ].is_executed = true) "why isn't it true?";
                  rob.entries[ rob_id ].is_executed := false;
                  next_state.core_[ j ].ROB_ := rob;
                  next_state.core_[ j ].LSQ_.entries[ i ].state := lsq_ld_st_fwd_branch;
                end;
              elsif (next_state.core_[ j ].LSQ_.entries[ squash_st_id ].state = lsq_ld_write_result) then
                if (next_state.core_[ j ].LSQ_.entries[ squash_st_id ].instruction.seq_num >= violating_seq_num) then
                  next_state.core_[ j ].LSQ_.entries[ i ].state := lsq_ld_st_fwd_branch;
                end;
               elsif (next_state.core_[ j ].LSQ_.entries[ squash_st_id ].state = lsq_await_load_mem_response) then
                if (next_state.core_[ j ].LSQ_.entries[ squash_st_id ].instruction.seq_num >= violating_seq_num) then
                  next_state.core_[ j ].LSQ_.entries[ i ].state := lsq_squashed_await_ld_mem_resp;
                end;
               elsif (next_state.core_[ j ].LSQ_.entries[ squash_st_id ].state = lsq_build_packet_send_mem_request) then
                if (next_state.core_[ j ].LSQ_.entries[ squash_st_id ].instruction.seq_num >= violating_seq_num) then
                  next_state.core_[ j ].LSQ_.entries[ i ].state := lsq_ld_st_fwd_branch;
                end;
              end;
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ replay_issue_load_to_mem ===> replay_generated_lsq_await_load_mem_response" 
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ replay_compare_and_check_state ===> lsq_st_await_committed" 
...
begin
  ...
  replay_value := Sta.core_[ j ].LSQ_.entries[ i ].read_value;
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_po ===> ROB_await_replay_complete" 
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_if_head ===> rob_commit_po || rob_commit_if_head" 
...
((Sta.core_[ j ].RENAME_.entries[ i ].state = issue_if_head)
&
(Sta.core_[ j ].IQ_.num_entries < IQ_NUM_ENTRIES_CONST))
