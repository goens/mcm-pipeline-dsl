...
INST_TYPE : enum {ld, st, mfence, inval};
...
begin
  ...
  for i : 0 .. LSQ_NUM_ENTRIES_ENUM_CONST do
    ...
    if (curr_entry.instruction.seq_num = seq_num) then
      assert ((curr_entry.state = lsq_squashed_await_ld_mem_resp) | (curr_entry.state = lsq_await_load_mem_response) | (curr_entry.state = replay_generated_lsq_await_load_mem_response)) "ASSN LQ: Should be in await mem resp? or squashed and await collect the mem resp?";
...
begin
  ...
  alias rename_c0 : init_state.core_[ 0 ].RENAME_ do
    ...
    rename_c0.entries[ 2 ].instruction.imm := 0;
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ lsq_ld_write_result ===> LoadReplay_await_replay_start || lsq_ld_st_fwd_branch || lsq_ld_st_fwd_branch" 
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ lsq_ld_st_fwd_branch ===> lsq_build_packet_send_mem_request || lsq_ld_write_result || lsq_st_await_committed" 
        ...
        if ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].phys_addr = next_state.core_[ j ].LSQ_.entries[ i ].phys_addr) & ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.seq_num > next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.op = ld))) then
          ...
          for ROB_squash_idx : ROB_idx_t do
            if true then
              if (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state = rob_await_executed) then
              elsif (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state = rob_clear_lsq_store_head) then
                ...
                if ((next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].instruction.op = ld) & (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].instruction.seq_num > violating_seq_num)) then
               ...
               elsif (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state = ROB_await_replay_complete) then
                if ((next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].instruction.op = ld) & (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].instruction.seq_num > violating_seq_num)) then
                  next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].is_executed := false;
                  next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state := rob_await_executed;
                end;
               elsif (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state = original_commit_rob_commit_po) then
                if ((next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].instruction.op = ld) & (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].instruction.seq_num > violating_seq_num)) then
                  next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].is_executed := false;
                  next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state := rob_await_executed;
                end;
               elsif (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state = ROB_Load_to_mfence_stall_original_commit_rob_commit_po_Load_mfence_Load) then
                if ((next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].instruction.op = ld) & (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].instruction.seq_num > violating_seq_num)) then
                  next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].is_executed := false;
                  next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state := rob_await_executed;
                end;
               elsif (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state = ROB_mfence_to_Load_stall_original_commit_rob_commit_po_Load_mfence_Load) then
                if ((next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].instruction.op = ld) & (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].instruction.seq_num > violating_seq_num)) then
                  next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].is_executed := false;
                  next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state := rob_await_executed;
                end;
          ...
          for LSQ_squash_idx : LSQ_idx_t do
            if true then
              if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state = lsq_st_await_committed) then
              ...
              elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state = lsq_ld_await_committed) then
                if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].instruction.seq_num >= violating_seq_num) then
                  for ROB_squash_idx : ROB_idx_t do
                    if true then
                      if (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state = rob_await_executed) then
                ...
                if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].instruction.seq_num >= violating_seq_num) then
                  next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state := lsq_squashed_await_ld_mem_resp;
                end;
               ...
               elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state = lsq_await_translation) then
               elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state = lsq_await_scheduled) then
               elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state = lsq_await_creation) then
               elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state = LoadReplay_await_replay_start) then
                if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].instruction.seq_num >= violating_seq_num) then
                  for ROB_squash_idx : ROB_idx_t do
                    if true then
                      if (next_state.core_[ j ].ROB_.entries[ ROB_squash_idx ].state = rob_await_executed) then
               ...
               elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state = replay_issue_load_to_mem) then
                if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].instruction.seq_num >= violating_seq_num) then
                  next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state := lsq_ld_st_fwd_branch;
                end;
               elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state = replay_generated_lsq_await_load_mem_response) then
                if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].instruction.seq_num >= violating_seq_num) then
                  next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state := lsq_squashed_await_ld_mem_resp;
                end;
               elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state = replay_compare_and_check_state) then
                if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].instruction.seq_num >= violating_seq_num) then
                  next_state.core_[ j ].LSQ_.entries[ LSQ_squash_idx ].state := lsq_squashed_await_ld_mem_resp;
                end;
              else
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ replay_issue_load_to_mem ===> replay_generated_lsq_await_load_mem_response || lsq_ld_st_fwd_branch || lsq_ld_st_fwd_branch" 
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ replay_compare_and_check_state ===> lsq_ld_await_committed || lsq_squashed_await_ld_mem_resp || lsq_squashed_await_ld_mem_resp" 
...
==>
  ...
  var replay_value : val_t;

begin
  next_state := Sta;
  old_load_value := next_state.core_[ j ].rf_.rf[ next_state.core_[ j ].LSQ_.entries[ i ].instruction.dest_reg ];
  replay_value := next_state.core_[ j ].LSQ_.entries[ i ].read_value;
  if (old_load_value != replay_value) then
    next_state.core_[ j ].rf_.rf[ next_state.core_[ j ].LSQ_.entries[ i ].instruction.dest_reg ] := replay_value;
  ...
  while ((ROB_offset <= ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ...
    if (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num = next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num) then
      if (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = ROB_await_replay_complete) then
        next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state := ROB_Load_to_mfence_stall_original_commit_rob_commit_po_Load_mfence_Load;
      else
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_po ===> ROB_await_replay_complete || ROB_Load_to_mfence_stall_original_commit_rob_commit_po_Load_mfence_Load || rob_await_executed" 
...
begin
  ...
  if (next_state.core_[ j ].ROB_.entries[ i ].instruction.op = ld) then
    ...
    next_state.core_[ j ].ROB_.entries[ i ].state := ROB_Load_to_mfence_stall_original_commit_rob_commit_po_Load_mfence_Load;
  end;
  Sta := next_state;
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_if_head ===> ROB_mfence_to_Load_stall_original_commit_rob_commit_po_Load_mfence_Load || rob_commit_if_head || rob_await_executed" 
...
begin
  ...
  if (next_state.core_[ j ].ROB_.head = i) then
    next_state.core_[ j ].ROB_.entries[ i ].state := ROB_mfence_to_Load_stall_original_commit_rob_commit_po_Load_mfence_Load;
  else
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB original_commit_rob_commit_po ===> rob_await_creation || rob_await_creation || rob_commit_time_await_st_mem_resp || rob_await_executed" 
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB ROB_Load_to_mfence_stall_original_commit_rob_commit_po_Load_mfence_Load ===> ROB_Load_to_mfence_stall_original_commit_rob_commit_po_Load_mfence_Load || original_commit_rob_commit_po || original_commit_rob_commit_po || rob_await_executed" 
(Sta.core_[ j ].ROB_.entries[ i ].state = ROB_Load_to_mfence_stall_original_commit_rob_commit_po_Load_mfence_Load)
==>
 
  var RENAME_is_in_state_set : boolean;
  var IQ_is_in_state_set : boolean;
  var LSQ_is_in_state_set : boolean;
  var ROB_is_in_state_set : boolean;
...
begin
  ...
  if (found_entry = false) then
    ...
    if ((next_state.core_[ j ].IQ_.entries[ found_idx ].state = iq_await_creation) | (next_state.core_[ j ].IQ_.entries[ found_idx ].state = iq_schedule_inst)) then
      IQ_is_in_state_set := true;
    else
      IQ_is_in_state_set := false;
    end;
  end;
  LSQ_is_in_state_set := false;
  LSQ_while_break := false;
  LSQ_found_entry := false;
  if (next_state.core_[ j ].LSQ_.num_entries = 0) then
    LSQ_while_break := true;
  ...
  is_instruction_on_any_state := (RENAME_is_in_state_set | (IQ_is_in_state_set | (LSQ_is_in_state_set | ROB_is_in_state_set)));
  if (next_state.core_[ j ].ROB_.entries[ i ].instruction.op = mfence) then
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB ROB_mfence_to_Load_stall_original_commit_rob_commit_po_Load_mfence_Load ===> ROB_mfence_to_Load_stall_original_commit_rob_commit_po_Load_mfence_Load || rob_commit_po || rob_commit_po || rob_await_executed" 
(Sta.core_[ j ].ROB_.entries[ i ].state = ROB_mfence_to_Load_stall_original_commit_rob_commit_po_Load_mfence_Load)
==>
...
begin
  ...
  while ((ROB_offset <= ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = mfence)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_creation) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_if_head) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_po) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = original_commit_rob_commit_po) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_executed) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = ROB_Load_to_mfence_stall_original_commit_rob_commit_po_Load_mfence_Load) | (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = ROB_mfence_to_Load_stall_original_commit_rob_commit_po_Load_mfence_Load))))))) then
        ROB_is_in_state_set := true;
      else
        ROB_is_in_state_set := false;
      end;
  ...
  for IQ_iter : IQ_idx_t do
    if (Sta.core_[ j ].IQ_.entries[ IQ_iter ].instruction.seq_num != 0) then
      if ((next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.op = mfence)) then
        if (found_entry = false) then
          found_element := (next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num - next_state.core_[ j ].IQ_.entries[ IQ_iter ].instruction.seq_num);
          found_idx := IQ_iter;
        else
  ...
  while ((LSQ_offset <= LSQ_difference) & ((LSQ_while_break = false) & (LSQ_found_entry = false))) do
    LSQ_curr_idx := ((LSQ_entry_idx + (LSQ_NUM_ENTRIES_CONST - LSQ_offset)) % LSQ_NUM_ENTRIES_CONST);
    if ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.op = mfence)) then
      if ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state = lsq_await_scheduled) | ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state = lsq_await_translation) | ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state = lsq_ld_st_fwd_branch) | (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state = lsq_await_creation)))) then
        LSQ_is_in_state_set := true;
      else
        LSQ_is_in_state_set := false;
      end;
  ...
  is_instruction_on_any_state := (RENAME_is_in_state_set | (ROB_is_in_state_set | (IQ_is_in_state_set | LSQ_is_in_state_set)));
  if (next_state.core_[ j ].ROB_.entries[ i ].instruction.op = ld) then
    if is_instruction_on_any_state then
      next_state.core_[ j ].ROB_.entries[ i ].state := ROB_mfence_to_Load_stall_original_commit_rob_commit_po_Load_mfence_Load;
    else
