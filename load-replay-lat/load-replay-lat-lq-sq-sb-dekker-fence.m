...
ROB_NUM_ENTRIES_ENUM_CONST : 2;
ROB_NUM_ENTRIES_CONST : 3;
IQ_NUM_ENTRIES_ENUM_CONST : 2;
IQ_NUM_ENTRIES_CONST : 3;
RENAME_NUM_ENTRIES_ENUM_CONST : 2;
RENAME_NUM_ENTRIES_CONST : 3;
...
LQ_state : enum {await_committed, write_result, await_mem_response, build_packet_send_mem_request, await_sb_fwd_check_response, await_fwd_check, await_translation, await_scheduled, await_creation, init_entry_with_default_state, LoadReplay_await_replay_start, replay_issue_load_to_mem, replay_generated_await_mem_response, replay_compare_and_check_state};
...
ROB_state : enum {rob_await_executed, rob_commit_if_head, rob_await_creation, init_rob_entry, ROB_await_replay_complete, original_commit_rob_commit_if_head, ROB_Store_Load_to_mfence_stall_rob_commit_if_head_Store_Load_mfence_Load, ROB_Store_Load_to_Load_stall_rob_commit_if_head_Store_Load_mfence_Load};
...
begin
  ...
  alias rename_c0 : init_state.core_[ 0 ].RENAME_ do
    ...
    rename_c0.entries[ 1 ].instruction.op := mfence;
    rename_c0.entries[ 1 ].instruction.seq_num := 2;
    rename_c0.entries[ 1 ].instruction.dest_reg := 0;
    rename_c0.entries[ 1 ].instruction.imm := 0;
    rename_c0.entries[ 1 ].instruction.write_value := 0;
    rename_c0.entries[ 2 ].instruction.op := ld;
    rename_c0.entries[ 2 ].instruction.seq_num := 3;
    rename_c0.entries[ 2 ].instruction.dest_reg := 1;
    rename_c0.entries[ 2 ].instruction.imm := 1;
    rename_c0.entries[ 2 ].instruction.write_value := 0;
    rename_c0.head := 0;
    rename_c0.tail := 3 % (CORE_INST_NUM + 1);
    rename_c0.num_entries := 3;
  end;
  alias rename_c1 : init_state.core_[ 1 ].RENAME_ do
    ...
    rename_c1.entries[ 1 ].instruction.op := mfence;
    rename_c1.entries[ 1 ].instruction.seq_num := 2;
    rename_c1.entries[ 1 ].instruction.dest_reg := 0;
    rename_c1.entries[ 1 ].instruction.imm := 0;
    rename_c1.entries[ 1 ].instruction.write_value := 0;
    rename_c1.entries[ 2 ].instruction.op := ld;
    rename_c1.entries[ 2 ].instruction.seq_num := 3;
    rename_c1.entries[ 2 ].instruction.dest_reg := 1;
    rename_c1.entries[ 2 ].instruction.imm := 0;
    rename_c1.entries[ 2 ].instruction.write_value := 0;
    rename_c1.head := 0;
    rename_c1.tail := 3 % (CORE_INST_NUM + 1);
    rename_c1.num_entries := 3;
  end;
  return init_state;
end;
...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ write_result ===> LoadReplay_await_replay_start || await_creation || await_creation" 
...
begin
  ...
  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ...
    if (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num = next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num) then
      if (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_executed) then
        ...
        next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state := ROB_Store_Load_to_mfence_stall_rob_commit_if_head_Store_Load_mfence_Load;
      else
  ...
  next_state.core_[ j ].LQ_.entries[ i ].state := LoadReplay_await_replay_start;
  Sta := next_state;
...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ build_packet_send_mem_request ===> await_mem_response || await_creation || await_creation" 
...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ await_sb_fwd_check_response ===> build_packet_send_mem_request || write_result || await_creation || await_creation" 
...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ await_fwd_check ===> await_sb_fwd_check_response || write_result || await_sb_fwd_check_response || write_result || await_creation || await_creation" 
...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ await_translation ===> await_fwd_check || await_creation || await_creation" 
...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ replay_issue_load_to_mem ===> replay_generated_await_mem_response || await_creation || await_creation" 
((Sta.core_[ j ].LQ_.entries[ i ].state = replay_issue_load_to_mem) & !(Sta.core_[ j ].mem_interface_.out_busy))
==>
...
begin
    ...
    if next_state.core_[ j ].load_address_table_.entries[ load_address_table_iter ].valid then
      ...
      if (next_state.core_[ j ].load_address_table_.entries[ load_address_table_iter ].lat_seq_num = next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num) then
        if (found_entry = false) then
          ...
          if ((next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num - next_state.core_[ j ].load_address_table_.entries[ load_address_table_iter ].lat_seq_num) < found_element) then
            found_element := (next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num - next_state.core_[ j ].load_address_table_.entries[ load_address_table_iter ].lat_seq_num);
            found_idx := load_address_table_iter;
          end;
        end;
  ...
  if (found_entry = false) then
    ...
    next_state.core_[ j ].mem_interface_.out_msg.seq_num := next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num;
    next_state.core_[ j ].mem_interface_.out_busy := true;
    next_state.core_[ j ].LQ_.entries[ i ].state := replay_generated_await_mem_response;
  end;
  Sta := next_state;
...
ruleset j : cores_t; i : LQ_idx_t do 
  rule "LQ replay_compare_and_check_state ===> await_committed || await_creation || await_creation" 
(Sta.core_[ j ].LQ_.entries[ i ].state = replay_compare_and_check_state)
==>
...
begin
  next_state := Sta;
  old_load_value := next_state.core_[ j ].rf_.rf[ next_state.core_[ j ].LQ_.entries[ i ].instruction.dest_reg ];
  if (old_load_value != next_state.core_[ j ].LQ_.entries[ i ].replay_value) then
    next_state.core_[ j ].rf_.rf[ next_state.core_[ j ].LQ_.entries[ i ].instruction.dest_reg ] := next_state.core_[ j ].LQ_.entries[ i ].replay_value;
    violating_seq_num := next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num;
    for load_address_table_squash_idx : load_address_table_idx_t do
      if next_state.core_[ j ].load_address_table_.entries[ load_address_table_squash_idx ].valid then
        if (next_state.core_[ j ].load_address_table_.entries[ load_address_table_squash_idx ].state = load_address_table_await_insert_remove) then
            ...
            while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
              ...
              if true then
                if (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].state = rob_await_executed) then
                      ...
                      while ((LQ_offset < LQ_difference) & ((LQ_while_break = false) & (LQ_found_entry = false))) do
                        ...
                        if true then
                          if (next_state.core_[ j ].LQ_.entries[ LQ_squash_curr_idx ].state = await_committed) then
                           ...
                           elsif (next_state.core_[ j ].LQ_.entries[ LQ_squash_curr_idx ].state = LoadReplay_await_replay_start) then
                            violating_seq_num := violating_seq_num;
                            squash_from_sq := squash_from_sq;
                            if squash_from_sq then
                              squash_include_seq_num := (next_state.core_[ j ].LQ_.entries[ LQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num);
                           ...
                           elsif (next_state.core_[ j ].LQ_.entries[ LQ_squash_curr_idx ].state = replay_issue_load_to_mem) then
                            violating_seq_num := violating_seq_num;
                            squash_from_sq := squash_from_sq;
                            if squash_from_sq then
                              squash_include_seq_num := (next_state.core_[ j ].LQ_.entries[ LQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num);
                           ...
                           elsif (next_state.core_[ j ].LQ_.entries[ LQ_squash_curr_idx ].state = replay_generated_await_mem_response) then
                            violating_seq_num := violating_seq_num;
                            squash_from_sq := squash_from_sq;
                            if squash_from_sq then
                              squash_include_seq_num := (next_state.core_[ j ].LQ_.entries[ LQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num);
                           ...
                           elsif (next_state.core_[ j ].LQ_.entries[ LQ_squash_curr_idx ].state = replay_compare_and_check_state) then
                            violating_seq_num := violating_seq_num;
                            squash_from_sq := squash_from_sq;
                            if squash_from_sq then
                              squash_include_seq_num := (next_state.core_[ j ].LQ_.entries[ LQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num);
                 ...
                 elsif (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].state = ROB_Store_Load_to_mfence_stall_rob_commit_if_head_Store_Load_mfence_Load) then
                  violating_seq_num := violating_seq_num;
                  if (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].instruction.seq_num > violating_seq_num) then
                    if (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].instruction.op = ld) then
                      squash_from_sq := false;
                 ...
                 elsif (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].state = ROB_Store_Load_to_Load_stall_rob_commit_if_head_Store_Load_mfence_Load) then
                  violating_seq_num := violating_seq_num;
                  if (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].instruction.seq_num > violating_seq_num) then
                    if (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].instruction.op = ld) then
                      squash_from_sq := false;
          ...
          error "Controller is not on an expected state for a msg: (squash) from: (LQ) to: (load_address_table)";
        end;
      end;
    endfor;
  end;
  ...
  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ...
    if (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num = next_state.core_[ j ].LQ_.entries[ i ].instruction.seq_num) then
      if (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = ROB_await_replay_complete) then
        next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state := original_commit_rob_commit_if_head;
      else
        error "Controller is not on an expected state for a msg: (replay_complete) from: (LQ) to: (ROB)";
      end;
                ...
                while ((LQ_offset < LQ_difference) & ((LQ_while_break = false) & (LQ_found_entry = false))) do
                  ...
                  if true then
                    if (next_state.core_[ j ].LQ_.entries[ LQ_squash_curr_idx ].state = await_committed) then
                      ...
                      -- put "Hit the ROB -> Squash LQ, LQ entry in await_translation case.\n";
                      violating_seq_num := violating_seq_num;
                      squash_from_sq := squash_from_sq;
                      -- put next_state.core_[ j ].LQ_.entries[ LQ_squash_curr_idx ].instruction.seq_num;
                      -- put violating_seq_num;
                      if squash_from_sq then
                        squash_include_seq_num := (next_state.core_[ j ].LQ_.entries[ LQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num);
                      else
                        squash_include_seq_num := (next_state.core_[ j ].LQ_.entries[ LQ_squash_curr_idx ].instruction.seq_num > violating_seq_num);
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_if_head ===> ROB_await_replay_complete || original_commit_rob_commit_if_head || rob_commit_if_head || rob_await_creation || rob_await_creation" 
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB original_commit_rob_commit_if_head ===> rob_await_creation || rob_commit_if_head || rob_await_creation || rob_await_creation || rob_await_creation || rob_await_creation" 
(Sta.core_[ j ].ROB_.entries[ i ].state = original_commit_rob_commit_if_head)
==>
...
begin
  ...
  if (next_state.core_[ j ].ROB_.entries[ i ].instruction.op = ld) then
    ...
    for remove_load_address_table_idx : load_address_table_idx_t do
      if next_state.core_[ j ].load_address_table_.entries[ remove_load_address_table_idx ].valid then
        if (next_state.core_[ j ].load_address_table_.entries[ remove_load_address_table_idx ].lat_seq_num = next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) then
          if remove_key_dest_already_found then
            error "Error: Found multiple entries with same key in remove_key API func";
          elsif !(remove_key_dest_already_found) then
            next_state.core_[ j ].load_address_table_.entries[ remove_load_address_table_idx ].valid := false;
            next_state.core_[ j ].load_address_table_.num_entries := (next_state.core_[ j ].load_address_table_.num_entries - 1);
            next_state.core_[ j ].load_address_table_.entries[ remove_load_address_table_idx ].state := load_address_table_await_insert_remove;
            next_state.core_[ j ].load_address_table_.entries[ remove_load_address_table_idx ].lat_seq_num := 0;
            next_state.core_[ j ].load_address_table_.entries[ remove_load_address_table_idx ].state := load_address_table_await_insert_remove;
          else
            error "Unreachable.. Just to make Murphi metaprogramming parser parse the stmts...";
          end;
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB ROB_Store_Load_to_mfence_stall_rob_commit_if_head_Store_Load_mfence_Load ===> ROB_Store_Load_to_mfence_stall_rob_commit_if_head_Store_Load_mfence_Load || ROB_Store_Load_to_Load_stall_rob_commit_if_head_Store_Load_mfence_Load || ROB_Store_Load_to_Load_stall_rob_commit_if_head_Store_Load_mfence_Load || rob_await_creation || rob_await_creation" 
(Sta.core_[ j ].ROB_.entries[ i ].state = ROB_Store_Load_to_mfence_stall_rob_commit_if_head_Store_Load_mfence_Load)
==>
...
begin
  ...
  if (found_entry = false) then
    ...
    if ((next_state.core_[ j ].SB_.entries[ found_idx ].state = sb_await_creation) | ((next_state.core_[ j ].SB_.entries[ found_idx ].state = SB_Store_to_Store_stall_sb_await_send_mem_req) | ((next_state.core_[ j ].SB_.entries[ found_idx ].state = sb_await_send_mem_req) | (next_state.core_[ j ].SB_.entries[ found_idx ].state = sb_await_mem_response)))) then
      SB_Store_is_in_state_set := true;
    else
      SB_Store_is_in_state_set := false;
    end;
  ...
  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = st)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_executed) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_if_head) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = original_commit_rob_commit_if_head) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_creation) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = ROB_Store_Load_to_mfence_stall_rob_commit_if_head_Store_Load_mfence_Load) | (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = ROB_Store_Load_to_Load_stall_rob_commit_if_head_Store_Load_mfence_Load)))))) then
        ROB_Store_is_in_state_set := true;
      else
        ROB_Store_is_in_state_set := false;
      end;
  ...
  while ((LQ_offset < LQ_difference) & ((LQ_while_break = false) & (LQ_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].instruction.op = ld)) then
      if ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_scheduled) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_translation) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_fwd_check) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_sb_fwd_check_response) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = build_packet_send_mem_request) | ((next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_mem_response) | (next_state.core_[ j ].LQ_.entries[ LQ_curr_idx ].state = await_creation))))))) then
  ...
  is_instruction_on_any_state := (RENAME_Store_is_in_state_set | (IQ_Store_is_in_state_set | (SQ_Store_is_in_state_set | (SB_Store_is_in_state_set | (ROB_Store_is_in_state_set | (RENAME_Load_is_in_state_set | (IQ_Load_is_in_state_set | (LQ_Load_is_in_state_set | ROB_Load_is_in_state_set))))))));
  if (next_state.core_[ j ].ROB_.entries[ i ].instruction.op = mfence) then
    if is_instruction_on_any_state then
      next_state.core_[ j ].ROB_.entries[ i ].state := ROB_Store_Load_to_mfence_stall_rob_commit_if_head_Store_Load_mfence_Load;
    else
      next_state.core_[ j ].ROB_.entries[ i ].state := ROB_Store_Load_to_Load_stall_rob_commit_if_head_Store_Load_mfence_Load;
    end;
  else
    next_state.core_[ j ].ROB_.entries[ i ].state := ROB_Store_Load_to_Load_stall_rob_commit_if_head_Store_Load_mfence_Load;
  end;
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB ROB_Store_Load_to_Load_stall_rob_commit_if_head_Store_Load_mfence_Load ===> ROB_Store_Load_to_Load_stall_rob_commit_if_head_Store_Load_mfence_Load || rob_commit_if_head || rob_commit_if_head || rob_await_creation || rob_await_creation" 
(Sta.core_[ j ].ROB_.entries[ i ].state = ROB_Store_Load_to_Load_stall_rob_commit_if_head_Store_Load_mfence_Load)
==>
...
begin
  next_state := Sta;
  RENAME_mfence_is_in_state_set := false;
  RENAME_while_break := false;
  RENAME_found_entry := false;
  if (next_state.core_[ j ].RENAME_.num_entries = 0) then
    RENAME_while_break := true;
  ...
  while ((RENAME_offset < RENAME_difference) & ((RENAME_while_break = false) & (RENAME_found_entry = false))) do
    RENAME_curr_idx := ((RENAME_entry_idx + (RENAME_NUM_ENTRIES_CONST - RENAME_offset)) % RENAME_NUM_ENTRIES_CONST);
    if ((next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].instruction.op = mfence)) then
      if (next_state.core_[ j ].RENAME_.entries[ RENAME_curr_idx ].state = issue_if_head) then
        RENAME_mfence_is_in_state_set := true;
      else
        RENAME_mfence_is_in_state_set := false;
      end;
      RENAME_found_entry := true;
  ...
  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.op = mfence)) then
      if ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_await_creation) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = rob_commit_if_head) | ((next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = ROB_Store_Load_to_mfence_stall_rob_commit_if_head_Store_Load_mfence_Load) | (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = ROB_Store_Load_to_Load_stall_rob_commit_if_head_Store_Load_mfence_Load)))) then
...
ruleset j : cores_t; i : RENAME_idx_t do 
  rule "RENAME issue_if_head ===> issue_if_head || issue_if_head || issue_if_head || issue_if_head || issue_if_head" 
(((Sta.core_[ j ].RENAME_.entries[ i ].state = issue_if_head) & (Sta.core_[ j ].IQ_.num_entries < IQ_NUM_ENTRIES_CONST)) & (Sta.core_[ j ].IQ_.num_entries < IQ_NUM_ENTRIES_CONST) & (Sta.core_[j].RENAME_.num_entries > 0))
==>
...
begin
  ...
  if ((next_state.core_[ j ].RENAME_.head = i) & (!((next_state.core_[ j ].RENAME_.num_entries = 0)) & !((next_state.core_[ j ].ROB_.num_entries = ROB_NUM_ENTRIES_CONST)))) then
    if (next_state.core_[ j ].RENAME_.entries[ i ].instruction.op = mfence) then
      ...
      if (next_state.core_[ j ].RENAME_.entries[ i ].instruction.op = mfence) then
        next_state.core_[ j ].ROB_.entries[ next_state.core_[ j ].ROB_.tail ].state := ROB_Store_Load_to_mfence_stall_rob_commit_if_head_Store_Load_mfence_Load;
      else
