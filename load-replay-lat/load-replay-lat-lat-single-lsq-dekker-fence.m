...
LSQ_state : enum {lsq_ld_await_committed, lsq_ld_write_result, lsq_await_load_mem_response, lsq_build_packet_send_mem_request, lsq_ld_fwd_branch, lsq_ld_await_translation, lsq_st_await_committed, lsq_st_fwd_branch, lsq_st_await_translation, lsq_await_scheduled, lsq_await_creation, init_default_state, LoadReplay_await_replay_start, replay_issue_load_to_mem, replay_generated_lsq_await_load_mem_response, replay_compare_and_check_state};
...
ROB_state : enum {rob_await_executed, rob_clear_lsq_store_head, rob_commit_time_await_st_mem_resp, rob_commit_po, rob_commit_if_head, rob_await_creation, init_rob_entry, ROB_await_replay_complete, original_commit_rob_commit_po, ROB_Store_Load_to_mfence_stall_rob_commit_po_Store_Load_mfence_Load, ROB_Store_Load_to_Load_stall_rob_commit_po_Store_Load_mfence_Load};
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ lsq_ld_write_result ===> LoadReplay_await_replay_start || lsq_await_creation || lsq_await_creation" 
...
begin
  ...
  next_state.core_[ j ].LSQ_.entries[ i ].state := LoadReplay_await_replay_start;
  Sta := next_state;
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ lsq_ld_fwd_branch ===> lsq_build_packet_send_mem_request || lsq_ld_write_result || lsq_await_creation || lsq_await_creation" 
(Sta.core_[ j ].LSQ_.entries[ i ].state = lsq_ld_fwd_branch)
==>
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ lsq_ld_await_translation ===> lsq_ld_fwd_branch || lsq_await_creation || lsq_await_creation" 
(Sta.core_[ j ].LSQ_.entries[ i ].state = lsq_ld_await_translation)
==>
...
begin
  ...
  next_state.core_[ j ].LSQ_.entries[ i ].state := lsq_ld_fwd_branch;
  Sta := next_state;
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ lsq_st_fwd_branch ===> lsq_st_await_committed || lsq_await_creation || lsq_await_creation" 
(Sta.core_[ j ].LSQ_.entries[ i ].state = lsq_st_fwd_branch)
==>
    ...
    while ((LSQ_offset < LSQ_difference) & ((LSQ_while_break = false) & (LSQ_found_entry = false))) do
      ...
      if ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].phys_addr = next_state.core_[ j ].LSQ_.entries[ i ].phys_addr) & ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.seq_num > next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.op = ld))) then
        ...
        while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
          ...
          if true then
            if (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].state = rob_await_executed) then
             ...
             elsif (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].state = original_commit_rob_commit_po) then
              violating_seq_num := violating_seq_num;
              if (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].instruction.seq_num >= violating_seq_num) then
                if (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].instruction.op = ld) then
                  for load_address_table_squash_idx : load_address_table_idx_t do
             ...
             elsif (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].state = ROB_Store_Load_to_mfence_stall_rob_commit_po_Store_Load_mfence_Load) then
              violating_seq_num := violating_seq_num;
              if (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].instruction.seq_num >= violating_seq_num) then
                if (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].instruction.op = ld) then
                  for load_address_table_squash_idx : load_address_table_idx_t do
             ...
             elsif (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].state = ROB_Store_Load_to_Load_stall_rob_commit_po_Store_Load_mfence_Load) then
              violating_seq_num := violating_seq_num;
              if (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].instruction.seq_num >= violating_seq_num) then
                if (next_state.core_[ j ].ROB_.entries[ ROB_squash_curr_idx ].instruction.op = ld) then
                  for load_address_table_squash_idx : load_address_table_idx_t do
        ...
        while ((LSQ_offset < LSQ_difference) & ((LSQ_while_break = false) & (LSQ_found_entry = false))) do
          ...
          if true then
            if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state = lsq_ld_await_committed) then
              violating_seq_num := violating_seq_num;
             ...
             elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state = lsq_ld_fwd_branch) then
              violating_seq_num := violating_seq_num;
              if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num) then
                LSQ_squash_remove_count := (LSQ_squash_remove_count + 1);
                next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state := lsq_await_creation;
              end;
             elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state = lsq_ld_await_translation) then
              violating_seq_num := violating_seq_num;
              if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num) then
                LSQ_squash_remove_count := (LSQ_squash_remove_count + 1);
                next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state := lsq_await_creation;
              end;
             elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state = lsq_st_await_committed) then
              violating_seq_num := violating_seq_num;
              if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num) then
                LSQ_squash_remove_count := (LSQ_squash_remove_count + 1);
                next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state := lsq_await_creation;
              end;
             elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state = lsq_st_fwd_branch) then
              violating_seq_num := violating_seq_num;
              if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num) then
                LSQ_squash_remove_count := (LSQ_squash_remove_count + 1);
                next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state := lsq_await_creation;
              end;
             elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state = lsq_st_await_translation) then
              violating_seq_num := violating_seq_num;
              if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num) then
                LSQ_squash_remove_count := (LSQ_squash_remove_count + 1);
                next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state := lsq_await_creation;
             ...
             elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state = LoadReplay_await_replay_start) then
              violating_seq_num := violating_seq_num;
              if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num) then
                LSQ_squash_remove_count := (LSQ_squash_remove_count + 1);
                next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state := lsq_await_creation;
              end;
             elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state = replay_issue_load_to_mem) then
              violating_seq_num := violating_seq_num;
              if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num) then
                LSQ_squash_remove_count := (LSQ_squash_remove_count + 1);
                next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state := lsq_await_creation;
              end;
             elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state = replay_generated_lsq_await_load_mem_response) then
              violating_seq_num := violating_seq_num;
              if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num) then
                LSQ_squash_remove_count := (LSQ_squash_remove_count + 1);
                next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state := lsq_await_creation;
              end;
             elsif (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state = replay_compare_and_check_state) then
              violating_seq_num := violating_seq_num;
              if (next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].instruction.seq_num >= violating_seq_num) then
                LSQ_squash_remove_count := (LSQ_squash_remove_count + 1);
                next_state.core_[ j ].LSQ_.entries[ LSQ_squash_curr_idx ].state := lsq_await_creation;
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ lsq_st_await_translation ===> lsq_st_fwd_branch || lsq_await_creation || lsq_await_creation" 
(Sta.core_[ j ].LSQ_.entries[ i ].state = lsq_st_await_translation)
==>
...
begin
  next_state := Sta;
  next_state.core_[ j ].LSQ_.entries[ i ].phys_addr := next_state.core_[ j ].LSQ_.entries[ i ].virt_addr;
  next_state.core_[ j ].LSQ_.entries[ i ].state := lsq_st_fwd_branch;
  Sta := next_state;
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ replay_issue_load_to_mem ===> replay_generated_lsq_await_load_mem_response || lsq_await_creation || lsq_await_creation" 
((Sta.core_[ j ].LSQ_.entries[ i ].state = replay_issue_load_to_mem) & !(Sta.core_[ j ].mem_interface_.out_busy))
==>
...
begin
  ...
  for load_address_table_iter : load_address_table_idx_t do
    if next_state.core_[ j ].load_address_table_.entries[ load_address_table_iter ].valid then
      if (next_state.core_[ j ].load_address_table_.entries[ load_address_table_iter ].lat_seq_num = next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num) then
        if (found_entry = false) then
          found_element := (next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num - next_state.core_[ j ].load_address_table_.entries[ load_address_table_iter ].lat_seq_num);
          found_idx := load_address_table_iter;
        else
          if ((next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num - next_state.core_[ j ].load_address_table_.entries[ load_address_table_iter ].lat_seq_num) < found_element) then
            found_element := (next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num - next_state.core_[ j ].load_address_table_.entries[ load_address_table_iter ].lat_seq_num);
            found_idx := load_address_table_iter;
          end;
        end;
  ...
  if (found_entry = false) then
    ...
    next_state.core_[ j ].mem_interface_.out_msg.seq_num := next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num;
    next_state.core_[ j ].mem_interface_.out_busy := true;
    next_state.core_[ j ].LSQ_.entries[ i ].state := replay_generated_lsq_await_load_mem_response;
  end;
  Sta := next_state;
...
ruleset j : cores_t; i : LSQ_idx_t do 
  rule "LSQ replay_compare_and_check_state ===> lsq_ld_await_committed || lsq_await_creation || lsq_await_creation" 
(Sta.core_[ j ].LSQ_.entries[ i ].state = replay_compare_and_check_state)
==>
...
begin
  next_state := Sta;
  old_load_value := next_state.core_[ j ].rf_.rf[ next_state.core_[ j ].LSQ_.entries[ i ].instruction.dest_reg ];
  if (old_load_value != next_state.core_[ j ].LSQ_.entries[ i ].replay_value) then
    next_state.core_[ j ].rf_.rf[ next_state.core_[ j ].LSQ_.entries[ i ].instruction.dest_reg ] := next_state.core_[ j ].LSQ_.entries[ i ].replay_value;
    violating_seq_num := next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num;
    for load_address_table_squash_idx : load_address_table_idx_t do
      if next_state.core_[ j ].load_address_table_.entries[ load_address_table_squash_idx ].valid then
        if (next_state.core_[ j ].load_address_table_.entries[ load_address_table_squash_idx ].state = load_address_table_await_insert_remove) then
          violating_seq_num := violating_seq_num;
          ...
          error "Controller is not on an expected state for a msg: (squash) from: (LSQ) to: (load_address_table)";
        end;
      end;
    endfor;
  end;
  ...
  while ((ROB_offset < ROB_difference) & ((ROB_while_break = false) & (ROB_found_entry = false))) do
    ...
    if (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].instruction.seq_num = next_state.core_[ j ].LSQ_.entries[ i ].instruction.seq_num) then
      if (next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state = ROB_await_replay_complete) then
        next_state.core_[ j ].ROB_.entries[ ROB_curr_idx ].state := original_commit_rob_commit_po;
      else
        error "Controller is not on an expected state for a msg: (replay_complete) from: (LSQ) to: (ROB)";
      end;
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_po ===> ROB_await_replay_complete || original_commit_rob_commit_po || rob_await_creation || rob_await_creation" 
(Sta.core_[ j ].ROB_.entries[ i ].state = rob_commit_po)
==>
...
begin
  ...
  if (next_state.core_[ j ].ROB_.entries[ i ].instruction.op = ld) then
    ...
    while ((LSQ_offset < LSQ_difference) & ((LSQ_while_break = false) & (LSQ_found_entry = false))) do
      LSQ_curr_idx := ((LSQ_entry_idx + (LSQ_NUM_ENTRIES_CONST - LSQ_offset)) % LSQ_NUM_ENTRIES_CONST);
      if (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.seq_num = next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) then
        if (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state = LoadReplay_await_replay_start) then
          next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction := next_state.core_[ j ].ROB_.entries[ i ].instruction;
          next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state := replay_issue_load_to_mem;
        else
          error "Controller is not on an expected state for a msg: (start_replay) from: (ROB) to: (LSQ)";
        end;
        LSQ_found_entry := true;
      end;
      if (LSQ_offset < LSQ_difference) then
    ...
    next_state.core_[ j ].ROB_.entries[ i ].state := original_commit_rob_commit_po;
  end;
  Sta := next_state;
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_if_head ===> ROB_Store_Load_to_mfence_stall_rob_commit_po_Store_Load_mfence_Load || rob_commit_if_head || rob_await_creation || rob_await_creation" 
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB original_commit_rob_commit_po ===> rob_await_creation || rob_await_creation || rob_commit_time_await_st_mem_resp || rob_await_creation || rob_await_creation" 
((Sta.core_[ j ].ROB_.entries[ i ].state = original_commit_rob_commit_po) & !(Sta.core_[ j ].mem_interface_.out_busy))
==>
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB ROB_Store_Load_to_mfence_stall_rob_commit_po_Store_Load_mfence_Load ===> ROB_Store_Load_to_mfence_stall_rob_commit_po_Store_Load_mfence_Load || ROB_Store_Load_to_Load_stall_rob_commit_po_Store_Load_mfence_Load || ROB_Store_Load_to_Load_stall_rob_commit_po_Store_Load_mfence_Load || rob_await_creation || rob_await_creation" 
(Sta.core_[ j ].ROB_.entries[ i ].state = ROB_Store_Load_to_mfence_stall_rob_commit_po_Store_Load_mfence_Load)
==>
...
begin
  ...
  while ((LSQ_offset < LSQ_difference) & ((LSQ_while_break = false) & (LSQ_found_entry = false))) do
    ...
    if ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.seq_num < next_state.core_[ j ].ROB_.entries[ i ].instruction.seq_num) & (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.op = st)) then
      if ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state = lsq_await_scheduled) | ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state = lsq_st_await_translation) | ((next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state = lsq_st_fwd_branch) | (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state = lsq_await_creation)))) then
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB ROB_Store_Load_to_Load_stall_rob_commit_po_Store_Load_mfence_Load ===> ROB_Store_Load_to_Load_stall_rob_commit_po_Store_Load_mfence_Load || rob_commit_po || rob_commit_po || rob_await_creation || rob_await_creation" 
(Sta.core_[ j ].ROB_.entries[ i ].state = ROB_Store_Load_to_Load_stall_rob_commit_po_Store_Load_mfence_Load)
==>
    ...
    if (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].instruction.seq_num = next_state.core_[ j ].IQ_.entries[ i ].instruction.seq_num) then
      if (next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state = lsq_await_scheduled) then
        ...
        if (next_state.core_[ j ].IQ_.entries[ i ].instruction.op = st) then
          ...
          next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state := lsq_st_await_translation;
        else
          if (next_state.core_[ j ].IQ_.entries[ i ].instruction.op = ld) then
            next_state.core_[ j ].LSQ_.entries[ LSQ_curr_idx ].state := lsq_ld_await_translation;
          end;
        end;
      else
