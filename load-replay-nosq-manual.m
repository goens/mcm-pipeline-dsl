...
ROB_state : enum {rob_complete_store, rob_wait_store_completed, rob_commit_based_on_inst, rob_commit_if_head, rob_await_is_executed, rob_await_creation, init_rob_entry, ROB_await_replay_complete, original_commit_rob_commit_based_on_inst};
...
memory_unit_sender_state : enum {memory_unit_receiver, memory_unit_stage_send, mem_unit_send_get_input, memory_unit_send_init, replay_issue_load_to_mem, replay_generated_memory_unit_receiver, replay_compare_and_check_state};
...
begin
  -- TODO: Adjust this function to accept
  next_state := Sta;
  ...
  elsif (Sta.core_[j].memory_unit_sender_.state = replay_generated_memory_unit_receiver) &
...
ruleset j : cores_t; i : ROB_idx_t do 
  rule "ROB rob_commit_based_on_inst ===> ROB_await_replay_complete || original_commit_rob_commit_based_on_inst" 
...
ruleset j : cores_t do 
  rule "memory_unit_sender replay_issue_load_to_mem ===> replay_generated_memory_unit_receiver" 
...
begin
  ...
  if (old_load_value != replay_value) then
    next_state.core_[ j ].rf_.rf[ next_state.core_[ j ].memory_unit_sender_.instruction.dest_reg ] := replay_value;
